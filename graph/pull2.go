...
func (s *TagStore) verifyManifest2(eng *engine.Engine, manifestBytes []byte) (*registry.ManifestData, bool, error) {
...
func (s *TagStore) CmdPull2(job *engine.Job) engine.Status {
	...
	if len(mirrors) == 0 && (isOfficial || endpoint.Version == registry.APIVersion2) {
		...
		if err := s.pullV2Repository2(job.Eng, r, job.Stdout, localName, remoteName, tag, sf, job.GetenvBool("parallel")); err == nil {
	...
	if err = s.pullRepository2(r, job.Stdout, localName, remoteName, tag, sf, job.GetenvBool("parallel"), mirrors); err != nil {
		return job.Error(err)
	}
...
func (s *TagStore) pullRepository2(r *registry.Session, out io.Writer, localName, remoteName, askedTag string, sf *utils.StreamFormatter, parallel bool, mirrors []string) error {
		...
		downloadImage := func(img *registry.ImgData) {
			...
			if mirrors != nil {
				for _, ep := range mirrors {
					...
					if is_downloaded, err = s.pullImage2(r, out, img.ID, ep, repoData.Tokens, sf); err != nil {
	...
	WriteStatus2(requestedTag, out, sf, layers_downloaded)
	return nil
}

func (s *TagStore) pullImage2(r *registry.Session, out io.Writer, imgID, endpoint string, token []string, sf *utils.StreamFormatter) (bool, error) {
	...
	layers_downloaded := false
	var numLayers int = len(history)
	var numDownloaded int = 0
	...
	for i := len(history) - 1; i >= 0; i-- {
		...
		if !s.graph.Exists(id) {
			...
			for j := 1; j <= retries; j++ {
				...
				err = s.graph.Register(img, layer)
				if terr, ok := err.(net.Error); ok && terr.Timeout() && j < retries {
					time.Sleep(time.Duration(j) * 500 * time.Millisecond)
					continue
				} else if err != nil {
...
func WriteStatus2(requestedTag string, out io.Writer, sf *utils.StreamFormatter, layers_downloaded bool) {
...
func (s *TagStore) pullV2Repository2(eng *engine.Engine, r *registry.Session, out io.Writer, localName, remoteName, tag string, sf *utils.StreamFormatter, parallel bool) error {
	...
	if tag == "" {
		...
		for _, t := range tags {
			if downloaded, err := s.pullV2Tag2(eng, r, out, localName, remoteName, t, sf, parallel); err != nil {
				return err
			} else if downloaded {
		...
		if downloaded, err := s.pullV2Tag2(eng, r, out, localName, remoteName, tag, sf, parallel); err != nil {
			return err
		} else if downloaded {
			layersDownloaded = true
		}
	...
	WriteStatus2(requestedTag, out, sf, layersDownloaded)
	return nil
}

func (s *TagStore) pullV2Tag2(eng *engine.Engine, r *registry.Session, out io.Writer, localName, remoteName, tag string, sf *utils.StreamFormatter, parallel bool) (bool, error) {
	...
	manifest, verified, err := s.verifyManifest2(eng, manifestBytes)
	if err != nil {
	...
	downloads := make([]downloadInfo2, len(manifest.FSLayers))

	var numLayers int = len(manifest.FSLayers)
	var numDownloaded int = 0
	var percent float32 = 0.0
	for i := len(manifest.FSLayers) - 1; i >= 0; i-- {
		...
		downloadFunc := func(di *downloadInfo2) error {
	...
	for i := len(downloads) - 1; i >= 0; i-- {
		...
		if d.downloaded {
			...
			d.tmpFile.Seek(0, 0)
			if d.tmpFile != nil {
				err = s.graph.Register(d.img, d.tmpFile)
